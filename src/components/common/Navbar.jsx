...
const Navbar = () => {
  ...
  return (
    <>
      <nav className="container mx-auto flex items-center justify-between py-4 px-6">
        ...
        <div className="flex items-center space-x-4">
          {user && user.role === "admin" && (
            <Link
              to="/admin"
              className="block rounded text-sm bg-black text-white px-2 py-1"
            >
              Admin
            </Link>
          )}
      ...
      <div
        className={`fixed top-0 w-3/4 left-0 sm:w-1/2 md:w-1/3 h-full bg-white shadow-lg transform transition-transform duration-300 z-50 ${
